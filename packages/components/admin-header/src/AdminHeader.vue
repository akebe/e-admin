...
<script>
  ...
  export default {
    ...
    computed: {
      breadcrumbs() {
        ...
        if (path !== this.home.path) {
          let _list = this.getBreadcrumbByMenuData(this.menuData, path);
          if (_list) {
            list.push(..._list);
          } else {
    ...
    methods: {
      getBreadcrumbByMenuData(ary, path, list = []) {
        for (let item of ary) {
          if (item.path === path) {
            list.push(item);
            return list;
          } else if (item.children) {
            const _list = this.getBreadcrumbByMenuData(item.children, path, [...list, item]);
